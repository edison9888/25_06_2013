...
@interface SiteViewController(PVT)<AllTilesListViewDelegate,LoginManagerDelegate,CameraViewDelegate,ConnectionDelegate,WindScrollViewDelegate,DataPrefetcherDelegate>
-(void)addTopTileView;
-(void)ClickedTileWithIndex:(int)index;
-(void)updateGraphValuesFromServer;
-(void)jsonParser_ParseSectionList:(NSData*)responseData;
-(void)jsonParser_parseSiteDetails:(NSData*)responseData;
...
-(void)loadView
{
  ...
  [self.navigationItem setHidesBackButton:YES animated:NO];
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tabbar.png"]];
  ...
  m_CameraScrollView=[[CameraScrollView alloc]initWithFrame:CGRectMake(0, 0,304, 250) CameraID:m_Site.CameraID PagingView:mPagingView canScrollRight:NO];
  ...
  m_TitleLabel.text=@"Section View";
...
- (void)updateGraphValuesFromServer
{
  ...
  //  m_GestureRecognizerView.userInteractionEnabled=NO;
  mPagingView.isEnabled=NO;
  ...
  if(isReloading)
  {
    m_parseStringListURL = [NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPhone-iPhone_DS.svc/JSON/GetSectionListforSite?sessionToken=%@&siteID=%@",kserverAddress,pSessionToken,m_Site.m_SiteId];    
    [urls addObject:m_parseStringListURL];
  }
  
  m_parseArrayDetailsURL = [NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPhone-iPhone_DS.svc/JSON/GetDetailedSiteInfo?sessionToken=%@&siteID=%@&startDate=%@&endDate=%@",kserverAddress,pSessionToken,m_Site.m_SiteId, [self getFormatedStringForDate:dailyGraphStartDate], [self getFormatedStringForDate:dailyGraphEndDate]];
  [urls addObject:m_parseArrayDetailsURL];
  
  m_parseTotalEnergyURL = [NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPhone-iPhone_DS.svc/JSON/GetDailySiteInfo?sessionToken=%@&siteID=%@&startDate=%@&endDate=%@",kserverAddress,pSessionToken,m_Site.m_SiteId, [self getFormatedStringForDate:weeklyGraphStartDate], [self getFormatedStringForDate:weeklyGraphEndDate]];
  [urls addObject:m_parseTotalEnergyURL];
  
  m_parseTopViewDataURL = [NSString stringWithFormat:@"%@/isisclient/GreenVolts-ISIS-BL-Web-Models-iPhone-iPhone_DS.svc/JSON/GetCurrentSiteInfo?sessionToken=%@&siteID=%@",kserverAddress,pSessionToken,m_Site.m_SiteId];
...
- (void)onWebserviceProcessorSuccess:(WebserviceProcessor*)processor
{
  ...
  if(m_parseStringListURL)
    [self jsonParser_ParseSectionList:[processor dataForURL:m_parseStringListURL]];  
  if(m_parseArrayDetailsURL)
    [self jsonParser_parseSiteDetails:[processor dataForURL:m_parseArrayDetailsURL]];
  if(m_parseTotalEnergyURL)
  ...
  //  m_GestureRecognizerView.userInteractionEnabled=YES;
...
{
  //printf("************************************************** sliderTapped\n");
  if(m_Slider.isTracking)
  {
    m_bTapped=NO;
    [self handleSlider:m_Slider];
    return;
  }
  if (m_Slider.highlighted)
  {
    if(mPreviousValue != m_Slider.value)
    {
      m_bTapped = YES;
      [self handleSlider:m_Slider];
    }
  ...
  CGPoint pt = [g locationInView: m_Slider];
  CGFloat percentage = pt.x / m_Slider.bounds.size.width;
  CGFloat delta = percentage * (m_Slider.maximumValue - m_Slider.minimumValue);
  CGFloat value = m_Slider.minimumValue + delta;
  if(value<m_Slider.value)
  {
    m_bTapped = YES;
    if([m_TitleLabel.text compare:totalEnergyTitle] == NSOrderedSame)
      [m_Slider setValue:m_Slider.value-7 animated:YES];
    else
      [m_Slider setValue:m_Slider.value-1 animated:YES];
    [self handleSlider:m_Slider];
    return;
  }
  if(value>m_Slider.value)
...
{  
  ...
  for(int i=0;i<[self.navigationController.navigationBar.subviews count];i++)
  {
    ...
    if([pView isKindOfClass:[UILabel class]])
    {
      ...
      [pLabel setText:m_Site.m_Name];
    }
  }
  //      self.title=m_Site.m_Name;    
  //  self.navigationItem.title=m_Site.m_Name;
  ...
  [[NSUserDefaults standardUserDefaults]setObject:@"2" forKey:@"ENTITY_TYPE"];
  [[NSUserDefaults standardUserDefaults]setObject:m_Site.m_SiteId forKey:@"ID"];
  [[NSUserDefaults standardUserDefaults]setObject:m_Site.m_Name forKey:@"ENTITY_NAME"];
...
//    }
  ...
  if(m_Backbutton){[m_Backbutton removeFromSuperview];}
  [[NSUserDefaults standardUserDefaults]setBool:NO forKey:@"REACHED_MAXIMIUM"];
...
-(void)addTopTileView
{
  if(!m_TopTileView)
  {
    m_TopTileView=[[TopTileView alloc]initWithFrame:CGRectMake(8, 10, 314, 120)];
    [self.view addSubview:m_TopTileView];
  }
  if(m_Site.AlarmCount==0)
  ...
  [m_TopTileView setTopTileDNI:[frmtr stringFromNumber:m_Site.m_CurrentDNI]];
  [m_TopTileView setTopTileTodayKWH:[frmtr stringFromNumber:m_Site.m_TodayEnergy]];
  [m_TopTileView setCurrentKWValue:m_Site.m_CurrentPower];
  
  NSString *temp;
  if(m_Site.m_Temperature != NULL)
  {
    float tempVal=[m_Site.m_Temperature floatValue];
    temp=[NSString stringWithFormat:@"%2.0f°C",tempVal];
  }
  else 
    temp=@"";
  
  if(m_Site.m_WeatherStatus != NULL)
  {
    NSString *pIconName=[NSString stringWithFormat:@"%@.png",m_Site.m_WeatherIcon];
    [m_TopTileView setTopTileWeatherImageName:pIconName];
    
    [m_TopTileView setTopTileWeatherDescription:[NSString stringWithFormat:@"   %@ %@",m_Site.m_WeatherStatus,temp]];
  }
  else 
    [m_TopTileView setTopTileWeatherDescription:@""];
  
  
  if(m_Site.m_WindDirection == NULL && m_Site.m_WindSpeed == NULL)
    [m_TopTileView setTopTileWindInfo:@""];
  else 
  {
    float windspeed=[m_Site.m_WindSpeed floatValue];
    [m_TopTileView setTopTileWindInfo:[NSString stringWithFormat:@"   %@ %2.1f m/s",m_Site.m_WindDirection,windspeed]];
  }
  
  [m_TopTileView setTopTileTime:[NSString stringWithFormat:@"%@",[DateTimeConverter getActualTimeStringFrom:m_Site.m_LocalDateTime]]];
  
  float scale=m_Site.ArrayCount*16;
  ...
  if([m_Site.m_CurrentPower floatValue]<=scale/2)
  {
    float angle=((110.0)*([m_Site.m_CurrentPower floatValue]))/(scale/2);
    float rotationAngle=250.0+angle;
    [m_TopTileView setTopTilePowerProductionNeedleRotationAngle:((rotationAngle*0.0174532925))];      
  }
  else 
  {
    float angle=((110.0)*([m_Site.m_CurrentPower floatValue]-(scale/2)))/(scale/2);
    [m_TopTileView setTopTilePowerProductionNeedleRotationAngle:((angle*0.0174532925))];            
  }
  
  if([m_Site.m_CurrentDNI floatValue]<=600)
  {
    float angle=((110.0)*([m_Site.m_CurrentDNI floatValue]))/600.0;
    float rotationAngle=250.0+angle;
  ...
  {
    float angle=((110.0)*([m_Site.m_CurrentDNI floatValue]-600.0))/600.0;
...
{  
  ...
  //printf("\nResponse Data for GetSectionListforSiteResult:\n%s\n",[responseString UTF8String]);
  ...
  if([rootResult count])
  {
    ...
    {      
      ...
      sectionArray=nil;
      [[NSUserDefaults standardUserDefaults]setObject:@"site" forKey:@"BACK_BUTTON"];
      [m_AllTilesListView fillScrollViewWithArray:m_Site.m_SectionArray];
    }
    else 
    {
...
{  
  ...
  //printf("\nResponse String for GetDetailedSiteInfoResult:\n%s\n",[responseString UTF8String]);
  
  NSDictionary *data=[responseString JSONValue];
  
  NSDictionary *userResult= [data objectForKey:@"GetDetailedSiteInfoResult"]; 
  
  NSArray *rootResult = [userResult objectForKey:@"RootResults"];
  NSMutableArray *siteDetailArray=[[NSMutableArray alloc]init];
  if([rootResult count])
  {
    NSDictionary *rootResultDict=[rootResult objectAtIndex:0];
    ...
    {      
      for(int i=0;i<[rootResult count];i++)
      {
        ...
        [siteDetailArray addObject:pEntityDetails];
        
        pEntityDetails=nil;
      }
      [m_PowerVsDNIView fillValuesFrom:siteDetailArray andArrayCount:m_Site.ArrayCount];
      [m_WindView fillValuesFrom:siteDetailArray];
    }
    else 
    {
      NSNumber *errorId=[rootResultDict objectForKey:@"ErrID"];
...
-(void)jsonParser_parseToatlEnergy:(NSData*)responseData
{
  ...
  //printf("\nResponse String for GetDailySiteInfoResult:\n%s\n",[responseString UTF8String]);
  
  NSDictionary *data=[responseString JSONValue];
  
  NSDictionary *userResult= [data objectForKey:@"GetDailySiteInfoResult"]; 
...
{  
  ...
  //printf("\nResponse String for GetCurrentSiteInfoResult:\n%s\n",[responseString UTF8String]);
  
  NSDictionary *data=[responseString JSONValue];
  
  NSDictionary *userResult= [data objectForKey:@"GetCurrentSiteInfoResult"]; 
  ...
  if([rootResult count])
  {
    ...
    {      
      NSObject *pAlarmCount=[siteDictinary objectForKey:@"AlarmCount"];
      if(![pAlarmCount isKindOfClass:[NSNull class]])
        m_Site.AlarmCount=[[siteDictinary objectForKey:@"AlarmCount"] intValue];
      
      NSObject *pArrayCount=[siteDictinary objectForKey:@"ArrayCount"];
      if(![pArrayCount isKindOfClass:[NSNull class]])
        m_Site.ArrayCount=[[siteDictinary objectForKey:@"ArrayCount"] intValue];
      
      NSObject *pCameraID=[siteDictinary objectForKey:@"CameraID"];
      if(![pCameraID isKindOfClass:[NSNull class]])
        m_Site.CameraID=[[siteDictinary objectForKey:@"CameraID"] intValue];
      
      NSObject *pSiteId=[siteDictinary objectForKey:@"ID"];
      if(![pSiteId isKindOfClass:[NSNull class]])
        m_Site.m_SiteId=[siteDictinary objectForKey:@"ID"];      
      
      NSObject *pCurrentDNI=[siteDictinary objectForKey:@"CurrentDNI"];
      if(![pCurrentDNI isKindOfClass:[NSNull class]])
        m_Site.m_CurrentDNI=[siteDictinary objectForKey:@"CurrentDNI"];
      
      NSObject *pCurrentPower=[siteDictinary objectForKey:@"CurrentPower"];
      if(![pCurrentPower isKindOfClass:[NSNull class]])
        m_Site.m_CurrentPower=[siteDictinary objectForKey:@"CurrentPower"];
      
      NSObject *pLocalDateTime=[siteDictinary objectForKey:@"LocalDateTime"];
      if(![pLocalDateTime isKindOfClass:[NSNull class]])
        m_Site.m_LocalDateTime=[siteDictinary objectForKey:@"LocalDateTime"];
      
      NSObject *pLocation=[siteDictinary objectForKey:@"Location"];
      if(![pLocation isKindOfClass:[NSNull class]])
        m_Site.m_Location=[siteDictinary objectForKey:@"Location"];
      
      NSObject *pName=[siteDictinary objectForKey:@"Name"];
      if(![pName isKindOfClass:[NSNull class]])
        m_Site.m_Name=[siteDictinary objectForKey:@"Name"];
      
      NSObject *pTemperature=[siteDictinary objectForKey:@"Temperature"];
      if(![pTemperature isKindOfClass:[NSNull class]])
        m_Site.m_Temperature=[[siteDictinary objectForKey:@"Temperature"] stringValue];
      
      NSString *pWindDirection=(NSString *)[siteDictinary objectForKey:@"WindDirection"];
      if(![pWindDirection isEqualToString:@""])
        m_Site.m_WindDirection=[siteDictinary objectForKey:@"WindDirection"];
      
      NSObject *pWindSpeed=[siteDictinary objectForKey:@"WindSpeed"];
      if(![pWindSpeed isKindOfClass:[NSNull class]])
        m_Site.m_WindSpeed=[siteDictinary objectForKey:@"WindSpeed"];
      
      NSObject *pTodayEnergy=[siteDictinary objectForKey:@"TodayEnergy"];
      if(![pTodayEnergy isKindOfClass:[NSNull class]])
        m_Site.m_TodayEnergy=[siteDictinary objectForKey:@"TodayEnergy"];
      
      NSObject *pUTCDateTime=[siteDictinary objectForKey:@"UTCDateTime"];
      if(![pUTCDateTime isKindOfClass:[NSNull class]])
        m_Site.m_UTCDateTime=[siteDictinary objectForKey:@"UTCDateTime"];
      
      NSObject *pUTCOffset=[siteDictinary objectForKey:@"UTCOffset"];
      if(![pUTCOffset isKindOfClass:[NSNull class]])
        m_Site.m_UTCOffset=[siteDictinary objectForKey:@"UTCOffset"];
      
      NSObject *pWeatherStatus=[siteDictinary objectForKey:@"WeatherStatus"];
      if(![pWeatherStatus isKindOfClass:[NSNull class]])
        m_Site.m_WeatherStatus=[siteDictinary objectForKey:@"WeatherStatus"];
      
      NSObject *pWeatherIcon=[siteDictinary objectForKey:@"WeatherIcon"];
      if(![pWeatherIcon isKindOfClass:[NSNull class]])
        m_Site.m_WeatherIcon=[siteDictinary objectForKey:@"WeatherIcon"];
      
      NSObject *pYTDEnergy=[siteDictinary objectForKey:@"YTDEnergy"];
      if(![pYTDEnergy isKindOfClass:[NSNull class]])
        m_Site.m_YTDEnergy=[siteDictinary objectForKey:@"YTDEnergy"];
...
{  
  ...
  [[SliderManager SharedSliderManager] handleSlider:m_Slider andIsDailyGraph:bGraphTypeDaily withRespectToObject:m_Site];
...
-(void)ClickedTileWithIndex:(int)index
{
  ...
  Section *pSection=[m_Site.m_SectionArray objectAtIndex:index];
  if(mDataPrefetcher)
  {
    mDataPrefetcher.delegate=nil;
    [mDataPrefetcher CancelConnection];
  ...
  [mDataPrefetcher GetClassToBeJumpedFrom:pSection];
  if(m_ActivityIndicatorView){[m_ActivityIndicatorView removeFromSuperview];m_ActivityIndicatorView=nil;}
