...
-(void)getAlarmCount;
-(void)invalidate;
-(void) startTimerAlarmsCountUpdate;
-(void) stopTimerAlarmsCountUpdate;
...
-(id)init 
{
  ...
  if((self = [super init]))
  {
    ...
    m_ActivityIndicatorView=nil;    
      ...
      self.tabBarItem.selectedImage = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle]pathForResource:@"Alert" ofType:@"png"]];
      self.tabBarItem.unselectedImage = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle]pathForResource:@"AlertDull" ofType:@"png"]];
...
- (void)dealloc 
{
  ...
  if(m_ActivityIndicatorView){[m_ActivityIndicatorView removeFromSuperview];m_ActivityIndicatorView=nil;}  
    ...
    [self stopTimerAlarmsCountUpdate];
    myTimer = nil;
...
- (void)setNotifications
{ 
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onPortfolioLevelNotification:) name:EnteredPortfolioLevel object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(exitFromPortfolioLevelNotification:) name:ExitedPortfolioLevel object:nil];
}

-(void)onPortfolioLevelNotification:(NSNotification *)info
{
    NSLog(@"onPortfolioLevelNotification"); 
     [self getAlarmCount];
}
-(void)exitFromPortfolioLevelNotification:(NSNotification *)info
{
    NSLog(@"exitFromPortfolioLevelNotification"); 
    [self stopTimerAlarmsCountUpdate];
    GreenVoltsAppDelegate *appDelegate = (GreenVoltsAppDelegate *)[[UIApplication sharedApplication] delegate];
    int alarmCount = 0;
    [appDelegate setTabTabBarBadge:alarmCount];
}
...
-(void) startTimerAlarmsCountUpdate
{
    ...
    myTimer = [NSTimer scheduledTimerWithTimeInterval: 30.0 target: self selector: @selector(getAlarmCount) userInfo: nil repeats: YES]; 
...
-(void) stopTimerAlarmsCountUpdate
{
    if (myTimer)
    {
        [myTimer invalidate];  //stopping timer
        myTimer = nil;
    }
}
...
-(void)connectToServer
{
  ...
  NSString *pEntityId=[[NSUserDefaults standardUserDefaults]objectForKey:@"ID"];
    ...
    NSLog(@"URL Request: %@",[NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPhone-iPhone_DS.svc/JSON/GetAlertsforEntity?sessionToken=%@&entityScope=%@&entityID=%@",kserverAddress,pSessionToken,pEntityType,pEntityId]);
    NSLog(@"URL Request: %@",[NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPhone-iPhone_DS.svc/JSON/GetOpenAlarms?sessionToken=%@",kserverAddress,pSessionToken]);
    
    
    if([pEntityType isEqualToString:@"1"]) //If Current entity level is portfolio
    {
        
        mURLConnection=[NSURLConnection connectionWithRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPhone-iPhone_DS.svc/JSON/GetOpenAlarms?sessionToken=%@",kserverAddress,pSessionToken]]] delegate:self];
    }
    else                                    // current entity level is site,section or array
    {
...
- (void)getAlarmCount  
{
    ...
    if(mURLConnection_AlarmCount) 
	{
		[mURLConnection_AlarmCount cancel];
		mURLConnection_AlarmCount=nil;
	}
    ...
    NSLog(@"URL Request: %@",[NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPad-iPad_DS.svc/JSON/GetOpenAlarmsCount?sessionToken=%@",kserverAddress,pSessionToken]);
    
    mURLConnection_AlarmCount=[NSURLConnection connectionWithRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/GreenVolts-ISIS-BL-Web-Models-iPad-iPad_DS.svc/JSON/GetOpenAlarmsCount?sessionToken=%@",kserverAddress,pSessionToken]]] delegate:self];
...
{  
  ...
  NSDictionary *data=[responseString JSONValue];
    ...
    {   
        ...
        //openalarms in portfolio level
        if([data objectForKey:@"GetOpenAlarmsCountResult"])
        {
            GreenVoltsAppDelegate *appDelegate = (GreenVoltsAppDelegate *)[[UIApplication sharedApplication] delegate];
            int alarmCount = 0;
            
            NSDictionary *userResult = [data objectForKey:@"GetOpenAlarmsCountResult"];
            NSArray *rootResult = [userResult objectForKey:@"RootResults"];
            
            if(rootResult && rootResult.count >= 1)
            {
                NSDictionary *firstObject = [rootResult objectAtIndex:0];

                NSObject *pAlarmCount=[firstObject objectForKey:@"Count"];
                if(![pAlarmCount isKindOfClass:[NSNull class]])
                    alarmCount = [[firstObject objectForKey:@"Count"] intValue];
                [appDelegate setTabTabBarBadge:alarmCount];
            ...
            [self startTimerAlarmsCountUpdate];
       
        }   
        else  //openalertentity in site,section,array level
        {
            if([data objectForKey:@"GetAlertsforEntityResult"])
                userResult = [data objectForKey:@"GetAlertsforEntityResult"];
            else if([data objectForKey:@"GetOpenAlarmsResult"])
                userResult = [data objectForKey:@"GetOpenAlarmsResult"];
            else
                userResult = nil;
